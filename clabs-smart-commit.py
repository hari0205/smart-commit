...
def format_commit_message(subject: str, time_spent: str, comment: Optional[str] = None, transition: Optional[str] = None) -> str:
    """
    ...
    jira_commands = [f"#{key} {value}" for key, value in [
        ('time', time_spent),
        ('comment', comment),
...
def append_jira_commands(jira_issue_key: str, time_spent: str, comment: Optional[str] = None, transition: Optional[str] = None) -> str:
...
def main()->None:
    ...
    subject = commit_msg.split("#time")[0].strip()  # Get everything before #time as the subject
    ...
    formatted_commit_msg = format_commit_message(subject, time_spent, comment, transition)

    # Step 5: Append the Jira issue key to the commit message
    formatted_commit_msg = f"{formatted_commit_msg} {jira_issue_key}"

    # Step 6: Write the formatted message back to the file
